package de.strikerbay.letsbuildatemplate.reference;

public class Reference {

    public static final String MOD_ID = "LetsBuildATemplate";
    public static final String MOD_NAME = "Lets Build A Template";
    public static final String MOD_VERSION = "1.7.10-1.0";
    public static final String FINGERPRINT = "";
    public static final String SERVER_PROXY_CLASS = "de.strikerbay.letsbuildatemplate.proxy.ServerProxy";
    public static final String CLIENT_PROXY_CLASS = "de.strikerbay.letsbuildatemplate.proxy.ClientProxy";

}
